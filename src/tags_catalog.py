"""Чтение config/controllable_tags.yaml.

Файл — это action space (что можно подать как Δu), а не список признаков
модели. Резолвинг колонки нужен только для того немногого, что явно
упоминается по tag_id (monotone, чёрный список анализаторов, таргет) —
поэтому здесь не проверяется, что тег есть в секции installations_tags:
она документация, суффикс колонки берётся из installations.
"""
