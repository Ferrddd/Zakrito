"""Виртуальные анализаторы качества (лист «ВАК», узел 24-2000 -> гидроочищенное ДТ).

Формулы перенесены из исходного optim_agent без изменений. Это line-fit модели,
а не физика: приоритет достоверности по ТЗ ЛИМС > ПАК > ВАК. Поэтому агент
оптимизации использует их ТОЛЬКО ОТНОСИТЕЛЬНО: сравнивает прогноз ВАК для
сценария с прогнозом ВАК для текущего режима (Δ), а не сверяет абсолютное
значение со спецификацией. Абсолютные значения не надёжны (например, в формуле
cloud point у F22 нет коэффициента — вероятно, опечатка в источнике), а Δ от
управляющих воздействий определяется только коэффициентами при них.

Имена тегов в формулах — теги установки 24-2000. В витрине колонки, общие для
АВТ и 24-2000, получают суффикс `_hdt` (T6, F9, T11, F14, F26 ...), остальные
остаются без суффикса — поэтому имена резолвятся по фактическим колонкам.
"""
