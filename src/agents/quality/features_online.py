"""Онлайн-сборка признаков для инференса.

Модель обучена на лаговых/оконных признаках (T5__lag6, T5__mean18, ...), а
оркестратор отдаёт агенту ОДНУ строку состояния. Поэтому агент хранит короткий
буфер истории, а здесь из буфера собирается ровно тот же вектор признаков, что
видела модель при обучении.

Чтобы не дублировать логику (и не получить train/serve skew), используются те же
функции, что и в train.build_dataset: add_engineered + add_lag_features из
src.data_pipeline.features. Список нужных колонок и их порядок берутся из
feature_spec.json, который пишет train.py.

Нюансы, вытекающие из features.py:
- лаги/окна считаются внутри block_id -> строки состояния ДОЛЖНЫ нести block_id
  (колонка витрины). Если его нет, считаем всё одним блоком и предупреждаем;
- load_rel в add_engineered делится на медиану ВСЕГО датафрейма — онлайн медиана
  по буферу была бы другой, поэтому фиксируем медиану обучения из feature_spec;
- add_lag_features по умолчанию запускает процессы joblib — онлайн n_jobs=1.
"""
