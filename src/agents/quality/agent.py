"""QualityAgent — реализация протокола Agent для прогноза серы в товарном ДТ.

Снаружи виден только контракт: capabilities() и evaluate(state[, overrides]) -> AgentReport.
Внутри: буфер истории -> онлайн-признаки -> квантильная модель + классификатор
нарушения -> калибровка -> отчёт с уверенностью, драйверами и отказами.

Что делает агент сверх «просто predict»:
- хранит историю (модель учится на лагах/окнах, а оркестратор шлёт одну строку);
- сам выбирает режим: with_analyzer (ПАК исправен) / blind (ПАК нет/заморожен/вне диапазона);
- отказывается (abstain) при нехватке истории/тегов и при протухшем ПАК;
- what-if: evaluate(state, overrides={"T5_hdt": 345}) — оценка сценария оптимизатора;
- поддерживает несколько горизонтов, для которых на диске есть обученные модели.
"""
